    ...
    .define(
        ...
        function (exports) {
            ...
            self.loading = function (elem) {
                elem.append(
                    (this.elemLoad = $(
                        '<i class="layui-anim layui-anim-rotate layui-anim-loop layui-icon layui-icon-loading aloha-loading"></i>'
                    ))
                )
            };
            ...
            self.modal.base = function (msg, params) {
                ...
                params = $.extend(
                    {
                        skin: 'layui-layer-admin-modal aloha-alert',
            ...
            self.tab = {
                ...
                minLeft: null,
                maxLeft: null,
                wrap: '.aloha-tabs-wrap',
                menu: '.aloha-tabs-menu',
                next: '.aloha-tabs-next',
                prev: '.aloha-tabs-prev',
                step: 200,
                init: function () {
                    var tab = this;
                    var btnCls = tab.wrap + ' .aloha-tabs-btn';

                    layui.dropdown.render({
                        elem: '.aloha-tabs-down',
                        click: function (name) {
                            if (name === 'all') {
                                tab.delAll();
                            }
                            ...
                            if (name === 'current') {
                                tab.del(layui.aloha.route.fileurl);
                            }
                            if (name === 'refresh') {
                                tab.refresh();
                            }
                    ...
                    $(document).on('click', btnCls, function (e) {
                        ...
                        if ($(e.target).hasClass('aloha-tabs-close')) {
                            tab.del(url)
                        } else {
                    ...
                    $('.aloha-tabs-hidden').addClass('layui-show');
                    this.isInit = true
                },
                has: function (url) {
                    var exists = false;
                ...
                delAll: function (type) {
                    var tab = this;
                    var menuBtnClas = tab.menu + ' .aloha-tabs-btn';
                ...
                del: function (url, backgroundDel) {
                    ...
                    var lay = '[lay-url="' + url + '"]';
                    var thisBody = $(
                        '#' + conf.containerBody + ' > .aloha-tabs-item' + lay
                    );
                    ...
                    if (backgroundDel === undefined) {
                        if (thisMenu.hasClass('aloha-tabs-active')) {
                            $(this.menu + ' li:last').click()
                        }
                    }
                },
                refresh: function (url) {
                    url = url || layui.aloha.route.fileurl;
                    if (this.has(url)) {
                        this.del(url, true);
                        self.renderTabs(url)
                    }
                ...
                clear: function () {
                    this.data = [];
                    this.isInit = false;
                    $(document).off('click', this.wrap + ' .aloha-tabs-btn')
                },
                change: function (route, callback) {
                    ...
                    var activeCls = 'aloha-tabs-active';
                    ...
                    if (existsTab) {
                        ...
                        layui.aloha.navigate(route.href)
                    } else {
                        self.loadHtml(fileurl, function (res) {
                            var htmlElem = $(
                                "<div><div class='aloha-tabs-item' lay-url='" +
                                fileurl +
                                "'>" +
                                res.html +
                                '</div></div>'
                            ...
                            tab.data.push(route);
                            layui.aloha.render(tab.tabMenuTplId);
                    ...
                    layui.aloha.sidebarFocus(route.href);
                    return false
                },
                onChange: function () {
                }
            ...
            self.renderLayout = function (callback, url) {
                ...
                self.render(url, function (res) {
                    ...
                    if (conf.viewTabs === true) {
                        self.containerBody.addClass('aloha-tabs-body')
                    }
                    layui.aloha.appBody = self.containerBody;
