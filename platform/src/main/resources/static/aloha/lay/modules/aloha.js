layui.extend({
    ...
    var $ = layui.jquery;
    // var $bread = $('#aloha-layout .aloha-breadcrumb');
    ...
    self.appBody = null;
    self.shrinkCls = 'aloha-sidebar-shrink';
    self.isInit = false;
        ...
        if ($.inArray(route.fileurl, conf.indPage) === -1) {
            ...
            if (view.containerBody == null) {
                ...
                view.renderLayout(function () {
                    //重新渲染导航
                    element.render('nav', 'aloha-sidebar');
                    //加载视图文件
                    loadRenderPage()
                })
            } else {
    ...
    self.get = function (url, params, success, loadIndex) {
        if (params) {
            params.invalidate_ie_cache = new Date().getTime();
        }
        $.get(url, params, function (r) {
    ...
    // ajax post请求
    self.post = function (url, params, success, loadIndex) {
        if (params) {
            params.invalidate_ie_cache = new Date().getTime();
        }
        $.post(url, params, function (r) {
    ...
    // ajax put请求
    self.put = function (url, params, success, loadIndex) {
        if (params) {
            params.invalidate_ie_cache = new Date().getTime();
        }
        $.ajax({
    ...
    // ajax patch请求 更新部分信息
    self.patch = function (url, params, success) {
        if (params) {
            params.invalidate_ie_cache = new Date().getTime();
        }
        $.ajax({
    ...
    // ajax patch请求 更新部分信息
    self.patch = function (url, params, success, loadIndex) {
        if (params) {
            params.invalidate_ie_cache = new Date().getTime();
        }
        $.ajax({
    ...
    self.delete = function (url, params, success, loadIndex) {
        if (params) {
            params.invalidate_ie_cache = new Date().getTime();
        }
        $.ajax({
